...
(function() {
    ...
    function debugLog(...args) {
        if (!DEEP_DEBUG_LOGS) return;
        console.log("[DEEP-DEBUG]", ...args);
    }
    ...
    function initAgent() {
        ...
        function getLiveWsUrl() {
            ...
            return `${wsProto}//${window.location.host}/api/agent/deep-live-ws`;
        }
        ...
        async function flushAgentTextParts() {
            const merged = (liveTurnTextParts || []).join(" ").replace(/\s+/g, " ").trim();
            liveTurnTextParts = [];
            if (!merged) {
                return;
            }

            // Voice-first mode: avoid cluttering chat with model text.
            if (SHOW_AGENT_TEXT_IN_CHAT && window.addChatEntry) {
                window.addChatEntry("Agent", merged);
                return;
            }

            // If model returned text but no audio chunks, synthesize fallback speech.
            if (!liveTurnReceivedAudio) {
                debugLog("no live audio received; using TTS fallback for text");
                setBotState('speaking');
                await speak(merged);
            }
        }
        ...
        async function handleLiveToolCall(calls) {
            ...
            debugLog("tool_call received", calls);
            for (const call of calls || []) {
                const name = call?.name || "";
                const args = call?.args || {};
                try {
                    if (name === "render_on_map" || name === "show_map_layer") {
                        if (window.renderCustomData) {
                            await window.renderCustomData(args.filename, args.layer_type);
                        }
                    } else if (name === "render_in_panel" || name === "show_chart") {
                        if (window.renderChart) {
                            window.renderChart(args.filename);
                        }
                    } else {
                        console.warn("Unknown frontend tool call:", name);
                        if (window.addChatEntry) {
                            window.addChatEntry("Error", `Unknown tool call: ${name}`);
                        }
                    }
                } catch (err) {
                    console.error("Frontend tool call error:", err);
                    if (window.addChatEntry) {
                        window.addChatEntry("Error", `Tool call failed (${name}): ${err?.message || err}`);
                    }
                }
            }
            // Frontend doesn't need to send response back to socket, the backend handles tool_responses for deep agent
        }
            ...
            await new Promise((resolve, reject) => {
                try {
                    ...
                    ws.onmessage = async (event) => {
                        ...
                        if (msg.type === "audio") {
                            debugLog("audio chunk");
                            liveTurnReceivedAudio = true;
                        ...
                        if (msg.type === "text") {
                            debugLog("text chunk", msg.text);
                            if (typeof msg.text === "string" && msg.text.trim()) {
                                liveTurnTextParts.push(msg.text.trim());
                            }
                            return;
                        }

                        if (msg.type === "status") {
                            const statusText = (msg.message || "").trim();
                            debugLog("status", msg);
                            if (statusText) {
                                setBotState('processing', statusText);
                                if (window.addChatEntry) {
                                    window.addChatEntry("System", statusText);
                                }
                            }
                            return;
                        }
                        ...
                        if (msg.type === "turn_complete") {
                            debugLog("turn_complete");
                            if (liveThinkingTicker) {
                                clearInterval(liveThinkingTicker);
                                liveThinkingTicker = null;
                            }
                            await flushAgentTextParts();
        ...
        async function processTranscript(transcript) {
            ...
            try {
                debugLog("processTranscript start", transcript);
                if (window.addChatEntry) {
                    window.addChatEntry("User", transcript);
                }
                ...
                await new Promise((resolve, reject) => {
                    ...
                    liveTurnReceivedAudio = false;
                    liveTurnHasToolCall = false;
                    liveSuppressPostToolText = false;
                    liveTurnTextParts = [];
                    liveTurnResolver = resolve;
                    liveTurnRejecter = reject;
                    if (liveThinkingTicker) clearInterval(liveThinkingTicker);
                    liveThinkingTicker = setInterval(() => {
                        if (!liveTurnInFlight) return;
                        const secs = Math.max(1, Math.floor((Date.now() - liveTurnStartedAt) / 1000));
                        const msg = `Still thinking... ${secs}s`;
                        setBotState('processing', msg);
                        debugLog(msg);
                    }, 5000);
                    liveSocket.send(JSON.stringify({ type: "user_text", text: transcript }));
                    debugLog("user_text sent");
                    setTimeout(() => {
                        if (!liveTurnInFlight) return;
                        liveTurnInFlight = false;
                        if (liveThinkingTicker) {
                ...
                const msg = `${err?.message || err}`.toLowerCase();
                console.warn("Live processing failed:", err);
                debugLog("processTranscript error", err);
                if (liveThinkingTicker) {
                    clearInterval(liveThinkingTicker);
                    liveThinkingTicker = null;
                }
                if (window.addChatEntry) {
                    window.addChatEntry("Error", "Agent connection failed.");
                }
                setBotState('', "Agent connection failed.");
                setTimeout(() => setBotState(''), 2000);
            }
        }
